...
export class EndUsersComponent implements OnInit {
  ...
  constructor(private endUserService: EndUserService, private tokenStorageService: TokenStorageService) { }
  ...
  retrieveUsers(): void {
    this.endUserService.getUserListing(this.displayName, this.provider, this.page, this.pageSize, this.sort).pipe(first()).subscribe(
      data => {
        console.log(data);
  ...
  deleteEndUser(id: bigint) {
    ...
    if (!getUser) { return; }
    this.endUserService.deleteEndUser(getUser.id).pipe(first()).subscribe(
      data => {
        console.log(data);
